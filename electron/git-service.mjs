...
async function repositoryRoot(directory) {
    ...
    let bare;
    try {
        bare = await git(workingDirectory, [
            "rev-parse",
            "--is-bare-repository",
        ]);
    } catch (error) {
    ...
    if (bare.toString("utf8").trim() === "true") {
        throw new Error(
            `Bare Git repositories are not supported: ${absolutePath}`,
        );
    }

    try {
        const root = await git(workingDirectory, [
            "rev-parse",
            "--show-toplevel",
        ]);
...
export async function getFileHistory(repositoryPath, filePath, snapshotHead) {
    ...
    for (let index = 0; index < tokens.length; ) {
        ...
        if (!token) continue;
        if (token === "COMMIT") {
            const [hash, shortHash, subject, author, email, date] =
                tokens.slice(index, index + 6);
            if (
                !HASH_PATTERN.test(hash ?? "") ||
                !date ||
                Number.isNaN(Date.parse(date))
        ...
        if (previousPath === undefined || revisionPath === undefined) {
            throw new Error(
                "Git returned an incomplete file status in history.",
            );
        }
