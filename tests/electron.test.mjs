...
test(
  ...
  async (t) => {
    ...
    assert.equal(
      await page.evaluate(() => typeof window.gitvolution?.chooseRepository),
      "function",
    );

    const screenshotDirectory =
      process.env.GITVOLUTION_SCREENSHOT_DIR ?? join(root, "test-results");
    ...
    const next = page.getByRole("region", { name: "Next commit", exact: true });
    ...
    await page.locator(".file-item").filter({ hasText: "empty.txt" }).click();
    await expect(current).toContainText("An empty page.");
    await expect(slider).toBeDisabled();
    await page.locator(".file-item").filter({ hasText: "image.bin" }).click();
    ...
    await page.getByRole("button", { name: "Select a file from disk" }).click();
    await expect(current.locator("code")).toContainText("Math.round");
    await slider.fill("1");
