...
test("read-only Git service", async (t) => {
    ...
    async function commit(subject, directory = repo) {
        const date = new Date(
            Date.UTC(2024, 0, 1, 0, 0, sequence++),
        ).toISOString();
    ...
    await t.test(
        ...
        async () => {
            ...
            const revisions = await Promise.all(
                history.map((entry) =>
                    getFileRevision(repo, entry.hash, entry.path),
                ),
            );
            assert.deepEqual(
    ...
    await t.test(
        ...
        async () => {
            ...
            assert.deepEqual(
                await getFileRevision(repo, deleted, renamed),
                missing,
            );
            assert.deepEqual(
                await getFileRevision(repo, initial, renamed),
                missing,
            );
            assert.deepEqual(
                await getFileRevision(repo, moved, original),
                missing,
            );
            assert.deepEqual(
    ...
    await t.test(
        ...
        async () => {
            ...
            for (const entry of history.slice(0, -1)) {
                assert.equal(
                    (await getFileRevision(directory, entry.hash, entry.path))
                        .content,
                    "unchanged file contents\n",
                );
            }
    ...
    await t.test(
        ...
        async () => {
            ...
            assert.deepEqual(
                await getFileRevision(repo, blobs, "invalid-utf8.dat"),
                {
                    content: null,
                    binary: true,
                    missing: false,
    ...
    await t.test(
        ...
        async () => {
            ...
            assert.deepEqual(
                await getFileRevision(repo, blobs, "large-binary.dat"),
                {
                    content: null,
                    binary: true,
                    missing: false,
    ...
    await t.test(
        ...
        async () => {
            ...
            assert.deepEqual(
                (await getFileHistory(repo, "--help")).map(
                    (entry) => entry.hash,
                ),
                [initial],
            );
    ...
    await t.test(
        ...
        async () => {
            ...
            await assert.rejects(
                getFileRevision(repo, initial, "src"),
                /not a file/,
            );
        },
    );
    ...
    await t.test("linked worktrees and detached HEADs are valid", async () => {
        ...
        assert.equal(
            (await getFileHistory(worktree, renamed))[0].hash,
            initial,
        );
        assert.equal(
            (await getFileRevision(join(worktree, "src"), initial, original))
                .content,
            originalText,
        );
    });
    ...
    await t.test(
        ...
        async () => {
            ...
            assert.equal(
                (await getFileHistory(sha256, "file.txt"))[0].hash,
                hash,
            );
            assert.equal(
    ...
    await t.test(
        ...
        async () => {
            ...
            for (const path of files) {
                ...
                assert.equal(
                    (await getFileHistory(unusual, path))[0].path,
                    path,
                );
            }
